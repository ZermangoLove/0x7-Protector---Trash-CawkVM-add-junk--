				...
				while (linkedListNode.Next != null)
				{
					...
					if (i != 0)
					{
						...
						if (!flag)
						{
							...
							{
								int num13 = array2[i];
								int num14 = ICore.Utils.RandomBigInt32();
								list3.Add(Instruction.Create(OpCodes.Ldloc, local));
								list3.Add(Instruction.CreateLdcI4(num14));
								list3.Add(Instruction.Create(OpCodes.Mul));
								list3.Add(Instruction.Create(OpCodes.Ldstr, Convert.ToString(num13 * num14 - num12)));
								list3.Add(Instruction.Create(OpCodes.Call, Method.Module.Import(typeof(int).GetMethod("Parse", new Type[1] { typeof(string) }))));
